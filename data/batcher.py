...
@curry
def prepro_fn_extract_trans(max_src_len, max_src_num, batch):
    # def prepro_one(sample):
    #     source_sents, extracts = sample
    #     tokenized_sents = tokenize(max_src_len, source_sents)[:max_src_num]
    #     cleaned_extracts = list(filter(lambda e: e < len(tokenized_sents),
    #                                    extracts))
    #     return source_sents, cleaned_extracts #tokenized_sents 
    # batch = list(map(prepro_one, batch))
    return batch
...
@curry
def convert_batch_extract_trans(batch):
    return myextract.get_batch_trans(batch) #, batch
